...
REDIPS.drag = (function () {
    var init, init_tables, enable_drag, enable_table, img_onmousemove, handler_onmousedown, handler_ondblclick, table_top, handler_onmouseup, handler_onmousemove, element_drop, element_deleted, cell_changed, handler_onresize, set_trc, set_position, setTdStyle, getTdStyle, box_offset, calculate_cells, getScrollPosition, autoscrollX, autoscrollY, clone_div, copy_properties, clone_limit, elementControl, get_style, find_parent, find_cell, save_content, relocate, empty_cell, shift_cells, move_object, delete_object, animation, get_table_index, get_position, row_opacity, row_empty, row_clone, row_drop, form_elements, normalize, has_childs, obj_margin = null,
        ...
        scroll_object, bgstyle_old, scrollable_container = [],
        tables = [],
        sort_idx, moved, cloned, cloned_id = [],
        currentCell = [],
        div_drag = null,
        div_box = null,
        pointer = {
        ...
        border = 'solid',
        border_disabled = 'dotted',
        opacity_disabled, trash_cname = 'trash',
        trash_ask = true,
        trash_ask_row = true,
        save_pname = 'p',
        drop_option = 'multiple',
        shift_option = 'horizontal1',
        multiple_drop = 'bottom',
        delete_cloned = true,
        ...
        animation_pause = 20,
        animation_step = 2,
        animation_shift = false,
        shift_after = true,
        an_counter = 0,
        clone_shiftKey = false,
        clone_shiftKey_row = false,
    ...
    init_tables = function () {
        var i, j, k, element, level, group_idx, tables_nodeList, nested_tables, td, rowspan;
    ...
    handler_onmousedown = function (e) {
        var evt = e || window.event,
            offset, mouseButton, position, X, Y;
    ...
    row_clone = function (el) {
        var table_mini, offset, row_obj, last_idx, empty_row = true,
            cr, div, i, j;
        if (el.nodeName === 'DIV') {
            div = el;
    ...
    row_drop = function (r_table, r_row, table_mini) {
        var tbl = tables[r_table],
            ts = tbl.rows[0].parentNode,
            animated = false,
            tr, rp, src, rowIndex, delete_srow, drop;
    ...
    handler_onmouseup = function (e) {
        var evt = e || window.event,
            target_table, r_table, r_row, mt_tr, X, Y, i, target_elements, target_elements_length;
        X = evt.clientX;
        Y = evt.clientY;
        ...
        if (cloned && mode === 'cell' && (table === null || row === null || cell === null)) {
            ...
            if (mode === 'row') {
                ...
                cloned_id[obj_old.id] -= 1;
                REDIPS.drag.myhandler_notcloned();
            } else if (cloned && REDIPS.drag.delete_cloned === true && (X < target_table.redips.offset[3] || X > target_table.redips.offset[1] || Y < target_table.redips.offset[0] || Y > target_table.redips.offset[2])) {
                obj.parentNode.removeChild(obj);
                cloned_id[obj_old.id] -= 1;
                REDIPS.drag.myhandler_notcloned();
    ...
    handler_onmousemove = function (e) {
        var evt = e || window.event,
            bound = REDIPS.drag.bound,
            sca, X, Y, deltaX, deltaY, i, scrollPosition;
        if (evt.touches) {
            X = pointer.x = evt.touches[0].clientX;
            Y = pointer.y = evt.touches[0].clientY;
        } else {
        ...
        if (X < div_box[1] && X > div_box[3] && Y < div_box[2] && Y > div_box[0] && edge.flag.x === 0 && edge.flag.y === 0 && (currentCell.containTable || (X < currentCell[3] || X > currentCell[1] || Y < currentCell[0] || Y > currentCell[2]))) {
            set_trc();
            cell_changed();
        }
    ...
    set_trc = function () {
        var previous, cell_current, row_offset, row_found, cells, empty, mark_found, only_found, single_cell, tos = [],
            X, Y, i;
        previous = function () {
            if (table_old !== null && row_old !== null && cell_old !== null) {
                table = table_old;
    ...
    autoscrollX = function (so) {
        var pos, old, scrollPosition, maxsp, edgeCrossed, X = pointer.x,
            Y = pointer.y;
        if (edge.flag.x > 0) {
            calculate_cells();
            set_trc();
    ...
    elementControl = function (evt) {
        var flag = false,
            srcName, classes, regex_nodrag = /\bnodrag\b/i;
        if (evt.srcElement) {
    ...
    enable_drag = function (enable_flag, el, type) {
        var i, j, k, divs = [],
            tbls = [],
            borderStyle, opacity, cursor, overflow, autoscroll, enabled, cb, handler1, handler2, position, regex_drag = /\bdrag\b/i,
    ...
    save_content = function (tbl, type) {
        var query = '',
            tbl_start, tbl_end, tbl_rows, cells, tbl_cell, cn, id, r, c, d, JSONobj = [],
            pname = REDIPS.drag.save_pname;
    ...
    move_object = function (ip) {
        var p = {
            ...
            x1, y1, w1, h1, x2, y2, w2, h2, row, col, dx, dy, pos, i, target;
