...
if exists (select ROUTINE_NAME from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'PlayersHandicapReport')
	drop procedure PlayersHandicapReport
GO

create procedure PlayersHandicapReport
	(@LeagueID int = 0
	,@SectionID int = 0
	,@ClubID int = 0
	,@ActionNeeded bit = 0
...
if @fromApplyHCaps=1
	...
	order by SectionID,Section,Team,convert(datetime,[Last Date])
...
exec PlayersHandicapReport 0, 0, 0, 1, 1